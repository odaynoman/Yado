...
pub mod z_order {
    ...
    /// Whether any OTHER window sits above `our` at a screen point.
    /// Walks the z-order top-down and stops at our own window — our
    /// click-through state is irrelevant because we skip by identity,
    /// not by transparency.
